...
function TranscriptCardImpl({
  ...
  useEffect(() => {
    ...
    setError(null);
    fetch(vttUrl)
      // Through the shared mapping, so a 401 on this text body signs the
      // user out like any other request instead of reading "failed to load"
      // inside a dead page.
      .then((res) => expectText(res, "failed to load transcript"))
      .then((text) => {
        if (!active) return;
  ...
  // cursorKey is what the stepper position BELONGS to: this transcript and this
  // term. A new search, or a new video under a mounted panel, starts at its own
  // first match rather than wherever the last one left the cursor.
  //
  // Carried on the state rather than reset by an effect. An effect would run
  // AFTER the render that changed `find`, so there would be one committed frame
  // holding the old position — long enough for the scroll below to jump to the
  // previous term's match and then jump again.
  // Joined with a newline, which neither a URL nor a typed search term can
  // contain, so no pair of them can collide on one key.
  const cursorKey = `${loaded ?? ""}\n${find}`;
  const [cursor, setCursor] = useState({ key: cursorKey, at: 0 });
  const at = cursor.key === cursorKey ? cursor.at : 0;

  // Clamped on READ: typing another letter can shrink the match list under a
  // cursor pointing past its end, and a frame rendering "8 / 3" is a frame of
  // lying. activeCue is the match's index in `cues`, or -1 for none.
  const activeAt = matches.length ? Math.min(at, matches.length - 1) : -1;
  const activeCue = activeAt < 0 ? -1 : matches[activeAt];

  // steps counts how many times the user asked to move, as opposed to where
  // they ended up. The scroll effect needs it: pressing Next on a transcript
  // with ONE match wraps that match to itself, leaving activeCue unchanged, and
  // an effect keyed on position alone would decide there was nothing to do —
  // while the row it is pointing at sits scrolled out of the 340px box.
  ...
  function step(delta: number) {
    ...
    const cur = Math.min(at, matches.length - 1);
    setCursor({
      key: cursorKey,
      at: (cur + delta + matches.length) % matches.length,
    });
    setSteps((n) => n + 1);
  }
  ...
  //
  // `open` is a dependency because folding the panel unmounts this box and
  // reopening it starts at scrollTop 0 — without it the counter would come back
  // reading "5 / 47" with that row nowhere on screen.
  useEffect(() => {
            ...
            <div className="searchbox">
              ...
              <input
                ...
                onKeyDown={(e) => {
                  ...
                  // An Enter that commits an IME candidate is not a request
                  // for the next match. Swallowing it would eat the commit
                  // AND move the cursor, on a keystroke that was never aimed
                  // at this control.
