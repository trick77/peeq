...
import { getSettings, updateSettings, putCookie, cookieHealth, getAPITokenStatus, createAPIToken } from "./settings";
...
describe("settings api", () => {
  it("getSettings GETs /api/settings", async () => {
    const f = vi.spyOn(globalThis, "fetch").mockResolvedValue(new Response(JSON.stringify(settingsBody), { status: 200 }));
  ...
  it("cookieHealth GETs /api/cookie/health", async () => {
    const f = vi.spyOn(globalThis, "fetch").mockResolvedValue(
      new Response(JSON.stringify({ status: "valid", present: true }), { status: 200 }),
    );
  ...
  it("createAPIToken POSTs to /api/settings/token and returns the plaintext token", async () => {
    const f = vi.spyOn(globalThis, "fetch").mockResolvedValue(
      new Response(JSON.stringify({ token: "secret-token", created_at: "2026-07-20T00:00:00Z" }), { status: 200 }),
    );
