...
describe("Channel", () => {
  ...
  it("the New tab reports 'Never checked' when the channel has no last_scanned_at", async () => {
    const user = userEvent.setup();
    vi.mocked(getChannel).mockResolvedValue(detail({ last_scanned_at: undefined }));
    vi.mocked(listPending).mockResolvedValue([]);
    render(<Channel channelId="UCa" onOpenVideo={() => {}} onBack={() => {}} />);
    await screen.findByText("Uncanny Expeditions");
  ...
  it("shows an error when loading the New tab's pending list fails", async () => {
    const user = userEvent.setup();
    vi.mocked(listPending).mockRejectedValue(new Error("failed to load pending"));
    render(<Channel channelId="UCa" onOpenVideo={() => {}} onBack={() => {}} />);
    await screen.findByText("Uncanny Expeditions");

    await user.click(screen.getByRole("tab", { name: /new/i }));

    expect(await screen.findByText("failed to load pending")).toBeInTheDocument();
  });

  it("shows an error when the New tab's scan request itself fails", async () => {
    const user = userEvent.setup();
    vi.mocked(scanChannel).mockRejectedValue(new Error("failed to schedule a check"));
    render(<Channel channelId="UCa" onOpenVideo={() => {}} onBack={() => {}} />);
    await screen.findByText("Uncanny Expeditions");
    await user.click(screen.getByRole("tab", { name: /new/i }));
    ...
    expect(await screen.findByText("failed to schedule a check")).toBeInTheDocument();
  });
  ...
  it("falls back to a 0-day retention window on the Archive tab when settings fail to load", async () => {
    ...
    vi.mocked(listVideos).mockResolvedValue([
      archiveVideo({ id: "v1", watched: true, watched_at: watchedLongAgo, favorite: false }),
    ]);
    render(<Channel channelId="UCa" onOpenVideo={() => {}} onBack={() => {}} />);
    await screen.findByText("Uncanny Expeditions");
  ...
  it("an error loading the channel is shown instead of a blank page", async () => {
    vi.mocked(getChannel).mockReset();
    vi.mocked(getChannel).mockRejectedValue(new Error("failed to load channel"));
    render(<Channel channelId="UCa" onOpenVideo={() => {}} onBack={() => {}} />);
  ...
  it("shows an error and leaves the button usable when toggling subscribe fails", async () => {
    ...
    const { unsubscribeChannel } = await import("../api/channels");
    vi.mocked(unsubscribeChannel).mockReset();
    vi.mocked(unsubscribeChannel).mockRejectedValue(new Error("subscribe toggle failed"));
    render(<Channel channelId="UCa" onOpenVideo={() => {}} onBack={() => {}} />);
    await screen.findByText("Uncanny Expeditions");

    await user.click(screen.getByRole("button", { name: /^subscribed$/i }));

    expect(await screen.findByText("subscribe toggle failed")).toBeInTheDocument();
  });

  it("an untracked channel's Track button calls addChannel and reloads as tracked", async () => {
    const user = userEvent.setup();
    vi.mocked(getChannel).mockResolvedValue(detail({ tracked: false, subscribed: false, pending_count: 0 }));
    const { addChannel } = await import("../api/channels");
    vi.mocked(addChannel).mockReset();
    vi.mocked(addChannel).mockResolvedValue({ id: "UCa", name: "Uncanny Expeditions", subscribed: false });
    render(<Channel channelId="UCa" onOpenVideo={() => {}} onBack={() => {}} />);
    await screen.findByText("Uncanny Expeditions");

    await user.click(screen.getByRole("button", { name: /track this channel/i }));

    await waitFor(() => {
      expect(addChannel).toHaveBeenCalledWith("https://www.youtube.com/channel/UCa", false);
    });
    await waitFor(() => expect(getChannel).toHaveBeenCalledTimes(2));
  });
  ...
  it("shows an error when tracking an untracked channel fails", async () => {
    ...
    expect(await screen.findByText("failed to add channel")).toBeInTheDocument();
  });

  it("renders a handle-less, untracked, description-less channel without crashing", async () => {
    vi.mocked(getChannel).mockResolvedValue(
      detail({ handle: undefined, description: undefined, tracked: false, subscribed: false, pending_count: 0 }),
    );
    render(<Channel channelId="UCa" onOpenVideo={() => {}} onBack={() => {}} />);
    await screen.findByText("Uncanny Expeditions");
  ...
  it("clicking a card's watched button on the Archive tab calls setWatched and updates optimistically", async () => {
    const user = userEvent.setup();
    vi.mocked(listVideos).mockResolvedValue([archiveVideo({ id: "v1", watched: false })]);
  ...
  it("typing in the Archive tab's search box refetches with the query", async () => {
    ...
    await waitFor(() => {
      expect(listVideos).toHaveBeenCalledWith(expect.objectContaining({ channel: "UCa", q: "abyss" }));
    });
  ...
  it("choosing a category on the Archive tab refetches with that category", async () => {
    ...
    await waitFor(() => {
      expect(listVideos).toHaveBeenCalledWith(expect.objectContaining({ channel: "UCa", category: "ai" }));
    });
  });
  ...
  it("choosing a sort option on the Archive tab refetches with that sort", async () => {
    ...
    await waitFor(() => {
      expect(listVideos).toHaveBeenCalledWith(expect.objectContaining({ channel: "UCa", sort: "oldest" }));
    });
  });
  ...
  it("adding a pending item on the New tab calls downloadPending and removes it from the list", async () => {
    const user = userEvent.setup();
    vi.mocked(listPending).mockResolvedValue([
      { video_id: "p1", channel_id: "UCa", channel_name: "Uncanny Expeditions", title: "A new upload", duration_seconds: 300, url: "https://youtu.be/p1", thumbnail_url: "https://img.example/p1.jpg" },
    ]);
    render(<Channel channelId="UCa" onOpenVideo={() => {}} onBack={() => {}} />);
    await screen.findByText("Uncanny Expeditions");
    await user.click(screen.getByRole("tab", { name: /new/i }));
    ...
    await waitFor(() => expect(screen.queryByText("A new upload")).not.toBeInTheDocument());
  });
  ...
  it("shows an error when adding a pending item fails", async () => {
    ...
    vi.mocked(downloadPending).mockRejectedValue(new Error("failed to download"));
    render(<Channel channelId="UCa" onOpenVideo={() => {}} onBack={() => {}} />);
    await screen.findByText("Uncanny Expeditions");
    await user.click(screen.getByRole("tab", { name: /new/i }));
    await screen.findByText("A new upload");
  ...
  it("declining the delete confirmation leaves the channel intact", async () => {
    ...
    await user.click(await screen.findByRole("button", { name: /delete channel and its 142 videos/i }));
  ...
  it("a failed delete shows the error and does not navigate back", async () => {
    ...
    const onBack = vi.fn();
    vi.spyOn(window, "confirm").mockReturnValue(true);
    vi.mocked(deleteChannel).mockRejectedValue(new Error("failed to delete channel"));
    render(<Channel channelId="UCa" onOpenVideo={() => {}} onBack={onBack} />);
    await screen.findByText("Uncanny Expeditions");
    await user.click(screen.getByRole("tab", { name: /settings/i }));
    ...
    expect(await screen.findByText("failed to delete channel")).toBeInTheDocument();
  ...
  it("blurring the format override field saves it when the value changed", async () => {
    ...
    await waitFor(() => {
      expect(updateChannel).toHaveBeenCalledWith("UCa", { format_override: "bestaudio" });
    });
  });
  ...
  it("subscribing from the Settings tab calls subscribeChannel", async () => {
    ...
    const settingsPanel = document.querySelector(".chan-settings") as HTMLElement;
    await user.click(within(settingsPanel).getByRole("button", { name: /^subscribe$/i }));
  ...
  it("an unsubscribed, tracked channel's Settings tab hides autodownload/format/scan rows", async () => {
    ...
    expect(screen.queryByLabelText(/add new videos automatically/i)).not.toBeInTheDocument();
    expect(screen.queryByLabelText(/format override/i)).not.toBeInTheDocument();
    const settingsPanel = document.querySelector(".chan-settings") as HTMLElement;
    expect(within(settingsPanel).getByRole("button", { name: /^subscribe$/i })).toBeInTheDocument();
  });

  it("a failed autodownload toggle reverts and shows the error", async () => {
    const user = userEvent.setup();
    vi.mocked(updateChannel).mockRejectedValue(new Error("failed to update channel"));
    render(<Channel channelId="UCa" onOpenVideo={() => {}} onBack={() => {}} />);
    await screen.findByText("Uncanny Expeditions");
    await user.click(screen.getByRole("tab", { name: /settings/i }));
    ...
    expect(await screen.findByText("failed to update channel")).toBeInTheDocument();
  });
});
