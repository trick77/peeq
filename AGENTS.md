...
- `make build` — full build → `bin/vark` (CGO_ENABLED=0)
...
- Module path `github.com/trick77/vark`. Go 1.25 (`go.mod`; Containerfile build stage uses `golang:1.25-alpine`).
...
- Runtime image is `debian:12-slim` (glibc, apt), **not** distroless — vark shells out to `ffmpeg` and
...
## Config
- All runtime config comes from `VARK_*` env vars — see `.env.example`.
...
## vark invariants (must hold in every feature that talks to YouTube)
